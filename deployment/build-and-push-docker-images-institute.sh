...
docker buildx build -t "${GITLAB_DOCKER_REGISTRY_URL}/institute/nginx:${IMAGE_TAG}" $PLATFORM_ARG --load -f ../institute/nginx-service/docker/Dockerfile ../institute/nginx-service
docker push "${GITLAB_DOCKER_REGISTRY_URL}/institute/nginx:${IMAGE_TAG}"
