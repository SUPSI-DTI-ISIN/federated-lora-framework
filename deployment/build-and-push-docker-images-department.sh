...
docker buildx build -t "${GITLAB_DOCKER_REGISTRY_URL}/department/mlflow-service:${IMAGE_TAG}" $PLATFORM_ARG --load -f ../department/mlflow-service/docker/Dockerfile --build-arg ENV_PATH="$SERVICES_ENV_PATH" --build-arg UV_INDEX_GITLAB_USERNAME="$UV_INDEX_GITLAB_USERNAME" --build-arg UV_INDEX_GITLAB_PASSWORD="$UV_INDEX_GITLAB_PASSWORD" ../department/mlflow-service
docker push "${GITLAB_DOCKER_REGISTRY_URL}/department/mlflow-service:${IMAGE_TAG}"

docker buildx build -t "${GITLAB_DOCKER_REGISTRY_URL}/department/institute-service:${IMAGE_TAG}" $PLATFORM_ARG --load -f ../department/institute-service/docker/Dockerfile --build-arg ENV_PATH="$SERVICES_ENV_PATH" --build-arg UV_INDEX_GITLAB_USERNAME="$UV_INDEX_GITLAB_USERNAME" --build-arg UV_INDEX_GITLAB_PASSWORD="$UV_INDEX_GITLAB_PASSWORD" ../department/institute-service
docker push "${GITLAB_DOCKER_REGISTRY_URL}/department/institute-service:${IMAGE_TAG}"

docker buildx build -t "${GITLAB_DOCKER_REGISTRY_URL}/department/federated-learning-management-service:${IMAGE_TAG}" $PLATFORM_ARG --load -f ../department/federated-learning-management-service/docker/Dockerfile --build-arg ENV_PATH="$SERVICES_ENV_PATH" --build-arg UV_INDEX_GITLAB_USERNAME="$UV_INDEX_GITLAB_USERNAME" --build-arg UV_INDEX_GITLAB_PASSWORD="$UV_INDEX_GITLAB_PASSWORD" ../department/federated-learning-management-service
docker push "${GITLAB_DOCKER_REGISTRY_URL}/department/federated-learning-management-service:${IMAGE_TAG}"

docker buildx build -t "${GITLAB_DOCKER_REGISTRY_URL}/frontend:${IMAGE_TAG}" $PLATFORM_ARG --load -f ../frontend/docker/Dockerfile --build-arg ENV_PATH="$FRONTEND_ENV_PATH" --build-arg NPM_TOKEN="$GITLAB_TOKEN" ../frontend
docker push "${GITLAB_DOCKER_REGISTRY_URL}/frontend:${IMAGE_TAG}"
