...
@pytest.fixture()
def app():
    with patch("institute_service.settings") as mock_settings, \
         patch("institute_service.lifespan", new=MagicMock()):
        mock_settings.cors_origins = ["http://localhost:3000"]
        from institute_service import create_app
        return create_app()
...
class TestCreateApp:
    ...
    def test_title(self, app):
        assert app.title == "Institute Service"
    ...
    def test_api_router_is_mounted_under_api_institute_prefix(self, app):
        assert any("/api_institute" in r.path for r in app.routes)
...
class TestMainBlock:
    ...
    def test_argument_parser_uses_default_port(self):
        parser = argparse.ArgumentParser()
        parser.add_argument("-p", "--port", type=int, default=9020)
        assert parser.parse_args([]).port == 9020
