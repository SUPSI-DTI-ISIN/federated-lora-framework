...
@pytest.fixture()
def app():
    with patch("model_service.lifespan", new=MagicMock()):
        from model_service import create_app
        return create_app()
...
class TestCreateApp:
    ...
    def test_title(self, app):
        assert app.title == "Model Service"

    def test_version(self, app):
        assert app.version == "1.0.0"

    def test_api_router_mounted_under_prefix(self, app):
        assert any("/api_model" in r.path for r in app.routes)
...
class TestMainBlock:
    ...
    def test_argument_parser_uses_default_port(self):
        parser = argparse.ArgumentParser()
        parser.add_argument("-p", "--port", type=int, default=8090)
        assert parser.parse_args([]).port == 8090
