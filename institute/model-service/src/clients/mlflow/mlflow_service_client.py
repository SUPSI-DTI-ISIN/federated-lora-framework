...
class MlFlowServiceClient(MlFlowServiceClientInterface):
    ...
    def __init__(self, mlflow_department_service_url: str):
        self.__mlflow_department_service_url = mlflow_department_service_url

    @classmethod
    def get_instance(cls, mlflow_department_service_url: str):
        if cls.__INSTANCE is None:
            cls.__INSTANCE = cls(mlflow_department_service_url=mlflow_department_service_url)
        return cls.__INSTANCE


    def get_model_base_manifest(self, model_key: str) -> ManifestDTO:
        model_base_manifest_url: str = f"{self.__mlflow_department_service_url}/model/{model_key}/manifest"
        try:
    ...
    def get_model_file(self, model_key: str, model_file_path: str) -> requests.models.Response:
        file_url = f"{self.__mlflow_department_service_url}/model/{model_key}/file_name/{model_file_path}"
        try:
    ...
    def get_adapters_version(self, model_key: str) -> ModelAdaptersVersionDTO:
        adapters_version_url: str = f"{self.__mlflow_department_service_url}/model/{model_key}/adapters"
        try:
    ...
    def get_adapter_manifest(self, model_key: str, adapter_version: int) -> ManifestDTO:
        adapter_manifest_url: str = f"{self.__mlflow_department_service_url}/model/{model_key}/adapters/{adapter_version}/manifest"
        try:
    ...
    def get_adapter_file(self, model_key: str, adapter_version: int, model_file_path: str) -> requests.models.Response:
        file_url = f"{self.__mlflow_department_service_url}/model/{model_key}/adapters/{adapter_version}/file_name/{model_file_path}"
        try:
            resp = requests.get(file_url, stream=True)
            resp.raise_for_status()
        except requests.exceptions.HTTPError as err:
