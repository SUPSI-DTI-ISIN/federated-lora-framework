...
from schemas.documents import DocumentDTO, TrainingSamplesDTO, UpdateDocumentTrainableRequestDTO
...
def _doc_model(id=1, number="DOC-001", title="Test Doc", is_trainable=False, sections=None):
    m = DocumentModel()
    m.id = id
    m.number = number
    m.title = title
...
class TestUploadData:
    async def test_raises_invalid_file_error_when_content_empty(self, service):
        with pytest.raises(InvalidFileError):
            await service.upload_data(file_content=b"")

    async def test_raises_invalid_file_error_when_content_is_none(self, service):
        with pytest.raises(InvalidFileError):
            await service.upload_data(file_content=None)
    ...
    async def test_raises_document_already_exists_when_duplicate(self, service, repo):
        ...
        with patch("services.documents.documents_service.PdfParserService", return_value=mock_parser), \
             patch("services.documents.documents_service.tempfile.NamedTemporaryFile") as mock_tmp, \
            ...
            with patch("services.documents.documents_service.Path") as mock_path_cls:
                mock_path = MagicMock()
                mock_path.exists.return_value = True
                mock_path_cls.return_value = mock_path

                with pytest.raises(DocumentAlreadyExistsError) as exc_info:
                    await service.upload_data(file_content=b"PDF content")
    ...
    async def test_saves_and_returns_dto_on_success(self, service, repo):
        ...
        with patch("services.documents.documents_service.PdfParserService", return_value=mock_parser), \
             patch("services.documents.documents_service.DocumentMapper.to_model", return_value=_doc_model()), \
             patch("services.documents.documents_service.tempfile.NamedTemporaryFile") as mock_tmp, \
             patch("services.documents.documents_service.os.remove"):
            mock_tmp_file = MagicMock()
            mock_tmp_file.__enter__ = MagicMock(return_value=mock_tmp_file)
            ...
            with patch("services.documents.documents_service.Path") as mock_path_cls:
                ...
                dto = await service.upload_data(file_content=b"PDF content")
