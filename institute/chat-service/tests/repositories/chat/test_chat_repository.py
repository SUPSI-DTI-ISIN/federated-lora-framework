import pytest
from datetime import datetime, timezone
from unittest.mock import AsyncMock, MagicMock
from sqlalchemy.exc import SQLAlchemyError

from entities import ChatModel
from repositories.chat.chat_repository import ChatRepository


@pytest.fixture()
def session():
    return AsyncMock()


@pytest.fixture()
def repo(session):
    return ChatRepository(db_session=session)


@pytest.fixture()
def chat_model():
    m = ChatModel()
    m.id = 1
    m.user_id = "user-abc"
    m.title = "Test Chat"
    m.is_doing_inference = False
    m.created_at = datetime(2024, 1, 1, tzinfo=timezone.utc)
    m.updated_at = datetime(2024, 1, 1, tzinfo=timezone.utc)
    return m


def _make_execute_result(items, *, use_first=False):
    mock_scalars = MagicMock()
    if use_first:
        mock_scalars.first.return_value = items
    else:
        mock_scalars.all.return_value = items
    mock_result = MagicMock()
    mock_result.scalars.return_value = mock_scalars
    return mock_result


class TestSaveChat:
    async def test_success(self, repo, session, chat_model):
        result = await repo.save_chat(chat_model=chat_model)

        session.add.assert_called_once_with(chat_model)
        session.commit.assert_awaited_once()
        session.refresh.assert_awaited_once_with(chat_model)
        assert result is chat_model

    async def test_rolls_back_on_error(self, repo, session, chat_model):
        session.commit = AsyncMock(side_effect=SQLAlchemyError("commit failed"))

        with pytest.raises(SQLAlchemyError):
            await repo.save_chat(chat_model=chat_model)

        session.rollback.assert_awaited_once()


class TestGetAllByUser:
    async def test_returns_list(self, repo, session):
        models = [ChatModel(), ChatModel()]
        session.execute = AsyncMock(return_value=_make_execute_result(models))

        result = await repo.get_all_by_user(user_id="user-abc")
        assert result == models

    async def test_returns_empty_list(self, repo, session):
        session.execute = AsyncMock(return_value=_make_execute_result([]))

        assert await repo.get_all_by_user(user_id="user-abc") == []

    async def test_propagates_sqlalchemy_error(self, repo, session):
        session.execute = AsyncMock(side_effect=SQLAlchemyError("query failed"))

        with pytest.raises(SQLAlchemyError):
            await repo.get_all_by_user(user_id="user-abc")


class TestGetById:
    async def test_returns_model_when_found(self, repo, session, chat_model):
        session.get = AsyncMock(return_value=chat_model)

        assert await repo.get_by_id(chat_id=1) is chat_model

    async def test_returns_none_when_not_found(self, repo, session):
        session.get = AsyncMock(return_value=None)

        assert await repo.get_by_id(chat_id=999) is None

    async def test_propagates_sqlalchemy_error(self, repo, session):
        session.get = AsyncMock(side_effect=SQLAlchemyError("get failed"))

        with pytest.raises(SQLAlchemyError):
            await repo.get_by_id(chat_id=1)


class TestDeleteChatByUser:
    async def test_success(self, repo, session, chat_model):
        await repo.delete_chat_by_user(chat_model=chat_model)

        session.delete.assert_awaited_once_with(chat_model)
        session.commit.assert_awaited_once()

    async def test_rolls_back_on_error(self, repo, session, chat_model):
        session.commit = AsyncMock(side_effect=SQLAlchemyError("delete failed"))

        with pytest.raises(SQLAlchemyError):
            await repo.delete_chat_by_user(chat_model=chat_model)

        session.rollback.assert_awaited_once()
